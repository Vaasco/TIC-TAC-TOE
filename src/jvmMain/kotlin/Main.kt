...
var board = Position.totalPositions.toMutableList()

enum class Symbol(val s: String) {
    CIRCLE("O"), CROSS("X")
}

data class Position(val number: Int, val symbol: String) {
    companion object {
...
fun main() {
    ...
    while (!isGameOver(board)) {
        ...
        if (!Position.totalPositions.any { numberPicked == it.number }) {
            ...
            if (position!!.symbol == Symbol.CROSS.s || position.symbol == Symbol.CIRCLE.s) {
